...
aboutCode = aboutCode.replace(/<style>\s*@media \(min-width: 768px\) \{\s*\.about-grid[\s\S]*?<\/style>\s*<section class="section about reveal page-offset"[^>]*>[\s\S]*?<\/section>/, aboutReplacement);
// fallback if the class was not exactly page-offset in regex
aboutCode = aboutCode.replace(/<style>\s*@media \(min-width: 768px\) \{\s*\.about-grid[\s\S]*?<\/style>\s*<section class="section about reveal" id="about"[^>]*>[\s\S]*?<\/section>/, aboutReplacement);
